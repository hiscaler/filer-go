...
| 类型                                                 | 说明                                                                                                  |
|----------------------------------------------------|-----------------------------------------------------------------------------------------------------|
| `string`                                           | `http://` / `https://` URL；`data:...;base64,...`；含路径分隔符、以 `.` 开头、绝对路径或带扩展名的路径视为**本地文件**；否则视为**纯文本** |
...
| **`Quality`**                   | 字段，默认 **100**；影响 **JPEG / WebP（有损）** 输出。                                                     |
| **`Body() ([]byte, error)`**    | 若已 **`Resize`/`Crop`**（存在 `rgba`）：按当前 **`Ext()`** 与 **`Quality`** 编码后返回；否则惰性读取并缓存**原始字节**副本。 |
